...
CREATE OR REPLACE PROCEDURE venda_produtos2(
	qtd number, prc out sys_refcursor) is
BEGIN
    IF qtd IS NULL THEN
        OPEN prc FOR SELECT P1.P_NOME AS "NOME PRODUTO 1", P1.C_NOME AS "CATEGORIA PRODUTO 1", P1.SC_NOME AS "SUBCATEGORIA PROTUTO 1", 
            P2.P_NOME AS "NOME PRODUTO 2", P2.C_NOME AS "CATEGORIA PRODUTO 2", P2.SC_NOME AS "SUBCATEGORIA PROTUTO 2", COUNT(*) AS "QUANTIDADE"
            FROM DADOS_PRODUTO P1, DADOS_PRODUTO P2, ITEMVENDA_DUP IV1, ITEMVENDA_DUP IV2
            WHERE IV1.ID_PRODUTO = P1.ID_PRODUTO AND IV2.ID_PRODUTO = P2.ID_PRODUTO
                AND IV1.ID_VENDA = IV2.ID_VENDA
            GROUP BY P1.P_NOME, P1.C_NOME, P1.SC_NOME, P2.P_NOME, P2.C_NOME, P2.SC_NOME
            ORDER BY COUNT(*) DESC;
    ELSE
        OPEN prc FOR SELECT P1.P_NOME AS "NOME PRODUTO 1", P1.C_NOME AS "CATEGORIA PRODUTO 1", P1.SC_NOME AS "SUBCATEGORIA PROTUTO 1", 
	            P2.P_NOME AS "NOME PRODUTO 2", P2.C_NOME AS "CATEGORIA PRODUTO 2", P2.SC_NOME AS "SUBCATEGORIA PROTUTO 2", COUNT(*) AS "QUANTIDADE"
